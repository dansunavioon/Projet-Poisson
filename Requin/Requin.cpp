...
Requin::Requin(SDL_Renderer* renderer, float x, float y)
    : renderer(renderer), position{static_cast<int>(x), static_cast<int>(y)}, velocity{0, 0}, texture(nullptr), angle(0.0)
{
    ...
    SDL_Surface* tempSurface = IMG_Load("image_poisson/SharkBoss.png");
    if (tempSurface) {
...
void Requin::update(const std::vector<Poisson>& poissons)
{
    chaseFish(poissons);
...
void Requin::draw(SDL_Renderer* renderer, int cameraX, int cameraY) const
{
    if (texture)
    {
        SDL_Rect renderQuad = {
...
void Requin::chaseFish(const std::vector<Poisson>& poissons)
{
...
void Requin::hunt(std::vector<Poisson>& poissons)
{
